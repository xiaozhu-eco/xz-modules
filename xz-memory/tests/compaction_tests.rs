use xz_memory::{CompactionStrategy, Confidence, Fact, FactCategory, InMemoryMemory, MemorySystem};
...
async fn test_compact_merge_similar_with_different_objects() {
    ...
    let result = memory.compact_facts("u1", CompactionStrategy::MergeSimilar).await.unwrap();
...
async fn test_compact_remove_low_confidence() {
    ...
    let result =
        memory.compact_facts("u1", CompactionStrategy::RemoveLowConfidence(0.5)).await.unwrap();
...
async fn test_compact_remove_old() {
    ...
    let result = memory.compact_facts("u1", CompactionStrategy::RemoveOld(500)).await.unwrap();
